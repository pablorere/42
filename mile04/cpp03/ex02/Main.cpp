...
int main(void) {
    ...
    std::cout << "==================================================" << std::endl;
    {
        ...
        // As with ScavTrap, ClapTrap is built before FragTrap
        FragTrap fragger("HighFiver");

        std::cout << "\n--- FragTrap Basic Actions ---" << std::endl;
        // Attack consumes energy as expected
        fragger.attack("Bandit");
        
        // Test hit points and repair with FragTrap values
        fragger.takeDamage(80);
        fragger.beRepaired(50);

        std::cout << "\n--- FragTrap Special Ability ---" << std::endl;
        // Invoke its exclusive ability
        fragger.highFivesGuys(); 
        
        std::cout << "\n--- Destruction sequence ---" << std::endl;
        // Check for leaks and destruction order (FragTrap -> ClapTrap)
    }
